"""Horizontal slice simulator for the current parafoil guidance/control stack.

Default scenario
----------------
- Start point : (E=0 m, N=0 m)
- Target point: (E=0 m, N=200 m)
- Vehicle     : N=100 m is fixed while E is swept by dx
...
def print_header(args: argparse.Namespace) -> None:
    print("=" * 108)
    print("0513control_hori.py - current guidance/control horizontal slice")
    print("=" * 108)
    ...
    print(f"Vehicle N  : {args.n_m:.1f} m")
    print(
        "E sweep    : "
        f"{args.e_start_m:.1f} m -> {args.e_end_m:.1f} m "
        f"(dE={args.de_m:.1f} m)"
    )
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Sweep E at fixed N and print current L1/control outputs."
    )
    ...
    parser.add_argument("--n-m", type=float, default=100.0)
    parser.add_argument("--e-start-m", type=float, default=-100.0)
    parser.add_argument("--e-end-m", type=float, default=100.0)
    parser.add_argument("--de-m", type=float, default=10.0)
...
def main() -> None:
    ...
    for e_m in x_sweep_values(args.e_start_m, args.e_end_m, args.de_m):
        case = SimulationCase(
            e_m=e_m,
            n_m=args.n_m,
