...
=======
## 좌표/부호 규약 (반드시 이 기준으로 판단)
- 지역 좌표계: (E, N) 순서
- K_delta = 1.0: yaw_rate ≈ K_delta * (left - right)
- L_DISTANCE 3밴드: HIGH=25m, BASE=15m, LOW=10m (고도 기준)
- Anti-windup: 조건부 적분 방식

## 리뷰 항목
1. **수치 안정성**: division-by-zero 가드, 각도 wrap 처리, NaN/Inf 전파 경로
2. **부호 일관성**: cross product, bearing 계산, 좌우 채널 부호
3. **적분 와인드업**: anti-windup 조건이 모든 분기에서 올바른지
4. **GPS 유효성 게이트**: `is_gps_valid`, `is_gps_jump` 조건이 충분한지
5. **타이밍**: 10Hz 루프 내 blocking call 여부
6. **figure-eight 패턴**: lobe 전환 타이밍과 yaw 목표 설정 로직

## 출력
항목별 OK/WARN/FAIL 판정 + WARN/FAIL 항목에 파일:줄, 구체적 위험 설명, 수정 제안.
>>>>>>> 8a94f4a (skill, agent, hanes)
