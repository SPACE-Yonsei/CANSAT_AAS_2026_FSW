...
=======
- 메시지 포맷: `sender|receiver|MsgID|data` 문자열 직렬화
- 앱 목록: Main(10), FlightLogic(11), Comm(12), Barometer(13), IMU(14), GPS(15),
           Distance(16), Electro(17), Camera(18), Motor(19)
- 주요 이슈 (아래 문서 참조):
  - restart_app 인자 시그니처 불일치 (Camera, Motor)
  - TLM CSV 필드 불일치 (distance 누락, GPS 정밀도)
  - Queue 블로킹 put (maxsize=1000, non-blocking 미사용)
  - systemd/startup 경로 불일치

## 개정 요청 수신 시 수행 절차
1. 현재 코드 상태를 읽어 실제 이슈 재확인
2. 개정 범위를 **최소 변경 원칙**으로 결정
3. 단계별 계획 작성 (각 단계가 독립적으로 테스트 가능하도록):
   - Phase 1: 버그 수정 (안전망 먼저)
   - Phase 2: 구조 개선 (IPC, 시그니처)
   - Phase 3: 기능 추가/개편
4. 각 Phase별 영향받는 파일과 예상 변경 라인 수 명시
5. 회귀 리스크가 높은 부분 경고

## 출력 형식
- Mermaid 다이어그램으로 변경 전/후 아키텍처 비교 (요청 시)
- Phase별 체크리스트 (마크다운 테이블)
- 브레이킹 체인지 목록
>>>>>>> 8a94f4a (skill, agent, hanes)
