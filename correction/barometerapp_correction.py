...
from Sensor_Barometer import barometer
...
def send_hk(Main_Queue : Queue):
    ...
    while BAROMETERAPP_RUNSTATUS:
        status = msgstructure.send_msg(Main_Queue, appargs.BarometerAppArg.AppID, appargs.HkAppArg.AppID, appargs.BarometerAppArg.MID_SendHK, str(BAROMETERAPP_RUNSTATUS))
        if status == False:
            events.LogEvent(appargs.BarometerAppArg.AppName, events.EventType.error, "Error when sending HK message")
        time.sleep(1)
    return
...
def read_barometer_data(barometer_instance):
    ...
    while BAROMETERAPP_RUNSTATUS:
        try:
            ...
            with OFFSET_MUTEX:
                # Check if barometer instance is still valid before reading
                if barometer_instance is None:
                    time.sleep(0.1)
                    continue
                    
                PRESSURE, TEMPERATURE, ALTITUDE = barometer.read_barometer(barometer_instance, BAROMETER_OFFSET)
        except (AttributeError, OSError, RuntimeError) as e:
            # Handle I2C errors during termination (I2C object may be already deinitialized)
            if not BAROMETERAPP_RUNSTATUS:
                # If we're terminating, just break silently
                break
            events.LogEvent(appargs.BarometerAppArg.AppName, events.EventType.error, f"Error reading barometer: {e}")
            time.sleep(0.1)
            continue
