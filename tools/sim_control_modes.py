...
try:
    from Sensor_Motor import motorapp  # noqa: E402
...
CSV_PATH = ROOT / "control_mode_matrix_current.csv"
DOC_PATH = ROOT / "docs" / "control_mode_matrix_current.md"
...
def _expected_l1_reason(mode: str, fail_reason: str) -> str:
    if mode == guidance.ControlMode.FAIL.value:
        return fail_reason
    if mode.startswith("GPS_TRACKING_"):
        return "GPS_NAV"
    if mode.startswith("DR_"):
        return mode
    return "UNKNOWN_MODE"
...
def run_one_case(case: dict[str, bool], now: float = NOW) -> dict[str, object]:
    ...
    row.update(
        ...
        l1_in_reason_expected=_expected_l1_reason(mode_name, nav.fail_reason),
        l1_out_valid=l1_out.valid,
        l1_out_reason=l1_out.reason,
        ...
        pid_enabled=ctrl_in.pid_enabled if ctrl_in is not None else False,
    )
    return row
...
def write_markdown(rows: list[dict[str, object]], path: Path = DOC_PATH) -> None:
    ...
    invalid_limits = [
        row for row in rows
        if not row["l1_out_valid"] and not math.isfinite(float(row["yaw_rate_limit_dps"]))
    ]
    mismatched_l1_reason = [
        row for row in rows if row["l1_in_reason"] != row["l1_in_reason_expected"]
    ]
    ...
    for mode in guidance.ControlMode:
        lines.append(f"| `{mode.value}` | {mode_counts.get(mode.value, 0)} |")
    lines.extend([
        "",
        f"Unreachable modes: {', '.join(missing) if missing else 'none'}",
        "",
    ...
    lines.extend([
        ...
        "## Contract Checks",
        "",
        f"- L1 reason mismatches: {len(mismatched_l1_reason)}",
        f"- Invalid L1Output rows with non-finite yaw-rate limit: {len(invalid_limits)}",
        f"- CSV: `{CSV_PATH.name}`",
        "",
    ])
