...
- Env hints: `BARO_I2C_ADDR`, `BARO_INIT_RETRIES`, `ELECTRO_I2C_ADDR`, `IMU_I2C_ADDR`, `GPS_I2C_ADDR`, `GPS_I2C_INIT_RETRIES`, `GPS_I2C_INIT_DELAY_SEC`, `GPS_USE_UART`, `GPS_DEVICE`, `GPS_BAUD`, `DISTANCE_I2C_ADDR`, `DISTANCE_TFL_TRIGGER`.
...
| GPS I2C `[Errno 5] Input/output error` at `0x42` | Fixed in FSW: u-blox **0xFD/0xFE** must be read as **two 1-byte** transactions. Retry with `GPS_I2C_INIT_RETRIES` / `GPS_I2C_INIT_DELAY_SEC` if the module is slow to boot. |
