...
class TestControlModeGains(unittest.TestCase):
    ...
    def test_case_3_dr_m_gb_closed(self):
        ...
        # DR FF는 GPS 곡선이 아니라 DR 전용 ref(DR_FF_REF_DPS)로 정규화되고
        # DR_FF_DELTA_LIMIT_DEG로 cap된다.
        ...
        self.assertNotEqual(out.delta_pid_deg, 0.0)
        # DR FF는 DR 전용 deadband/ref로 계산된 뒤 DR cap으로 제한된다(코드와 동일식).
        ...
        expected_pre = control.angular_velocity_to_delta_ff(
            ...
            deadband_dps=config.DR_CTRL_ANGULAR_VELOCITY_DEADBAND_DEG_S,
            ref_dps=config.DR_FF_REF_DPS) * config.DR_M_FF_SCALE
        ...
        # 민감도: 같은 명령에서 (구) GPS-공유 곡선 값보다 더 큰 deflection.
        self.assertGreater(out.delta_ff_pre_cap_deg, gps.delta_ff_deg * config.DR_M_FF_SCALE)
